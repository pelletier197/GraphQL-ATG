...
export function generateArgsForField(field, typesByName, config) {
    return field.args.map((argument) => generateInputParameter(argument, typesByName, config));
}
function generateInputParameter(input, typesByName, config) {
    return {
        name: input.name,
...
function generateInput(input, typesByName, config) {
    ...
    return findMostSpecificFactory(input.type, typesByName, config)({
        ...context,
...
function findMostSpecificFactory(argumentType, typesByName, config, nullable = true) {
    ...
    if (isNonNull(argumentType)) {
        return findMostSpecificFactory(unwrapNonNull(argumentType), typesByName, config, false);
    }
    ...
    if (isList(argumentType)) {
        const listElementFactory = findMostSpecificFactory(unwrapList(argumentType), typesByName, config);
...
function randomFactory(argumentType, typesByName, config) {
    ...
    if (argumentType.kind === Kind.INPUT_OBJECT) {
        ...
        return () => {
            return _.mapValues(_.keyBy(fields, (field) => field.name), (input) => {
...
function findWildCardFactory(name, config) {
    const matchingKey = Object.keys(config.factories).find((key) => micromatch.isMatch(name, key));
    if (matchingKey) {
